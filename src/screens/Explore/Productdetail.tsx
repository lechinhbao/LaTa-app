...
const Productdetail = (props: NativeStackHeaderProps) => {
  ...
  const handle = ({ productID, colorProduct }: any) => {
    ...
    if (colorProduct == undefined) {
      ...
      if (checkAddProductColor.includes(colorProduct._id)) {
        Alert.alert('Thông báo', 'Sản phẩm đã có trong giỏ hàng!', [
          { text: 'OK' }
        ]);
      } else {
        dispatch(addItem({ key: uid(3), productID: productID,  colorProduct: colorProduct, quantity: 1 }));
        setHandleAdd(true);
      }
  ...
  const handleAddTocart = async () => {
    const cart: { key: any, productID: any; colorProduct: any; quantity: number }[] = [];
    data.map((item: any) =>
      cart.push({ key: item.key, productID: item.productID._id,  colorProduct: item.colorProduct._id, quantity: 1 })
    )
    ...
    <View style={{ height: '100%' }}>
      ...
      <View style={styles.addtocartButtonContainer}>
        {isUser == '' ?
          ...
          <TouchableOpacity
            style={styles.addtocartButton}
            onPress={() => { handle({ productID: product, colorProduct: selectedColor }); }}
          >
            <LinearGradient colors={['#46CAF3', '#68B1D9']} style={{ borderRadius: 10 }}>
