  ...
  const updateCart = async () => {
    ...
    listProduct.map(async (item: any) => {
      await AxiosInstance().put(
        `/product/updateQuantityProduct/${item.productID}`,
        {quantityOfOrder: item.quantityProduct},
      );
    });
            ...
            <View style={{width: '80%', gap: 5}}>
              <Text style={styles.textTitleItem}>
                {item.productID.productName.length < 20
                  ...
                  : item.productID.productName.substring(0, 20) + '...'}
              </Text>
              <View
                style={{
                  flexDirection: 'row',
  ...
  const RadioButton = ({selected}: {selected: boolean}) => {
    return (
      <View
        style={[
          styles.radioButton,
          {backgroundColor: selected ? '#40BFFF' : 'transparent'},
        ]}>
      ...
      <Pressable onPress={handlePress}>
        <View style={styles.paymentItemContainer}>
          ...
          {paymentMethod.linkIcon ? (
            <Image
              source={{uri: paymentMethod.linkIcon}}
    ...
    <SafeAreaView
      ...
      <ScrollView
        ...
        <View style={styles.itema}>
          <View style={{flexDirection: 'row'}}>
            <TextInput
              style={[
                ...
                {
                  borderColor:
                    receiverName.trim() !== '' || isReceiverNameValid
                      ? '#E5E5E5'
                      : 'red',
                },
              ]}
              ...
              <Pressable
                onPress={() =>
                  navigation?.navigate('Account', {
                    screen: RootStackScreenEnumAccount.AddressScreen,
                  })
                }>
          ...
          <Text
            style={{fontSize: 15, fontFamily: 'Poppins', fontWeight: '700'}}>
        ...
        <View style={styles.itemTotalPrice}>
          <Text style={styles.textBottomTotalLeft}>
            Tổng tiền (+ tiền giao hàng)
          </Text>
          <NumericFormat
            displayType={'text'}
            value={Number(totalAfterShipping)}
            allowLeadingZeros
            thousandSeparator=","
            renderText={(formattedValue: any) => (
