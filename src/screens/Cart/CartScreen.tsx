...
const CartScreen = ({navigation}: PropsCart) => {
  ...
  const totalItem = listData.reduce(
    (total: any, item: {quantity: any}) => total + item.quantity,
    0,
  );

  const generalPrice = listData.reduce(
    (previousValue: number, currentItem: any) =>
      previousValue + currentItem.productID?.price * currentItem.quantity,
    0,
  );
  ...
  const shipping =
    generalPrice < 3000000 ? generalPrice * 0.05 : generalPrice * 0.02;
  ...
  const handleApplyCoupon = async () => {
    try {
      const response = await AxiosInstance().get(`promotion/getAllPromotion`);
      const appliedPromotion = response.data.find(
        (promo: {discountCode: string}) => promo.discountCode === coupon,
      );

      if (appliedPromotion) {
        const discountLevelValue = appliedPromotion.discountLevel;
        const discountAmount =
          (generalPriceAfterShipping * discountLevelValue) / 100;
  ...
  const handleRemoveData = async () => {
    listData.map((item: any) => {
      cart.push({
        ...
        sizeProduct: item.sizeProduct._id,
        colorProduct: item.colorProduct._id,
        quantity: 1,
      });
    });
    await AxiosInstance().post('/users/updateInfoUser', {
      _id: user._idUser,
      cartItem: cart,
    });
      ...
      <View style={styles.itemCart}>
        ...
        <View style={{flexDirection: 'column', height: '100%'}}>
            ...
            <View style={{width: '65%', gap: 10}}>
              <Text style={styles.textTitleItem}>
                {item.productID.productName.length < 15
                  ...
                  : item.productID.productName.substring(0, 15) + '...'}
              </Text>
                ...
                <View style={{flexDirection: 'row'}}>
                  ...
                  <View
                    style={{
                      width: 20,
                      height: 20,
                      backgroundColor: `${item.colorProduct.code}`,
                      borderRadius: 50,
                    }}></View>
                </View>
              </View>
          ...
          <View style={styles.bottomItem}>
            <NumericFormat
              displayType={'text'}
              value={Number(item.productID.price)}
              allowLeadingZeros
              thousandSeparator=","
              renderText={(formattedValue: any) => (
                <Text style={styles.textPrice}>{formattedValue + 'đ'} </Text>
              )}
            />
            <View
              style={{
              ...
              <Pressable
                onPress={() => changeQuantityDown()}
                style={
                  quantity > 1
                    ? styles.btnNumberCountMinus
                    : [styles.btnNumberCountMinus, {backgroundColor: '#E5E5E5'}]
                }>
                <Icon name="remove-outline" size={25} />
              </Pressable>
              <Text style={styles.textNumberCount}>{item.quantity}</Text>
              <Pressable
                onPress={() => changeQuantityUp()}
                style={
                  quantity < 10
                    ? styles.btnNumberCountPlus
                    : [styles.btnNumberCountPlus, {backgroundColor: '#E5E5E5'}]
                }>
  ...
  return isUser != '' ? (
      ...
      <ScrollView
        ...
        <View style={{height: HEIGHT * 0.8}}>
          <View
            style={{
              borderWidth: 1,
              borderColor: isInvalidCoupon ? 'red' : inputBorderColor,
              borderRadius: 5,
              marginTop: 25,
            }}>
            <InputItem
          ...
          <View style={styles.itemTotalPrice}>
            <View style={styles.headerTotalPrice}>
              <Text style={styles.textHeaderTotalLeft}>
                Tổng giá sản phẩm ({totalItem})
              </Text>
              <NumericFormat
                displayType={'text'}
                value={Number(generalPrice)}
                allowLeadingZeros
                thousandSeparator=","
                renderText={(formattedValue: any) => (
            ...
            <View style={styles.headerTotalPrice}>
              ...
              <Text style={styles.textHeaderTotalRight}>
                {isVoucherApplied ? `${discountLevel}%` : '0%'}
              </Text>
            </View>
            <View style={styles.bottomTotalPrice}>
              ...
              <NumericFormat
                ...
                thousandSeparator=","
                renderText={(formattedValue: any) => (
                  <Text style={styles.textBottomTotalRight}>
                    {formattedValue + 'đ'}{' '}
                  </Text>
                )}
              />
          ...
          <View style={{marginTop: 15}}>
            <Pressable
              onPress={() =>
                listData.length > 0
                  ? navigations.navigate('CartDetail', {
                      Level: isVoucherApplied ? discountLevel : '0',
                      totalAfterShipping: isVoucherApplied
                        ? discountedPrice
                        : generalPriceAfterShipping,
                      generalPriceAfterShipping,
                    })
    ...
    <View>
      <Image
        style={{alignSelf: 'center', marginTop: '30%', width: 300, height: 300}}
        source={require('../../asset/image/nodata.png')}
      />
      <Text
        style={{
          alignSelf: 'center',
