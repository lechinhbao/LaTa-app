    ...
    useEffect(() => {
        const layDuLieu = async () => {
            try {
                ...
                const response = await AxiosInstance().get('https://vapi.vnappmob.com/api/province');
                
                const data = response.data.results.map((item: any) => {
        
                    return { key: item.province_id + " " + item.province_name, value: item.province_name }
                })
                setTinhThanhs(data);
            } catch (error) {
        ...
        if (tinhThanhSelected) {
            const layQuanHuyen = async () => {
                try {
                    ...
                    const response = await AxiosInstance().get(`https://vapi.vnappmob.com/api/province/district/${tinhThanhSelected}`);

                    const data = response.data.results?.map((item: any) => {
                        return { key: item.district_id + ' ' + item.district_name, value: item.district_name }
                    })
                    setQuans(data);
                } catch (error) {
        ...
        if (quanSelected) {
            const layPhuongXa = async () => {
                try {
                    ...
                    const response = await AxiosInstance().get(`https://vapi.vnappmob.com/api/province/ward/${quanSelected}`);
                    const data = response.data.results?.map((item: any) => {
                        return { key: item.ward_id + ' ' + item.ward_name, value: item.ward_name }
                    })
                    setPhuongs(data);
                } catch (error) {
